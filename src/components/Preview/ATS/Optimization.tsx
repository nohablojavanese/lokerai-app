...
const ATSAnalyzer: React.FC<ATSAnalyzerProps> = ({ cv }) => {
  ...
  const analyzeATS = () => {
    ...
    const keywords = [
      ...
      'project management', 'analytical skills', 'customer service',
      'time management', 'adaptability', 'creativity'
    ];
    const cvText = JSON.stringify(cv).toLowerCase();
    const keywordsFound = keywords.filter(keyword => cvText.includes(keyword));
    ...
    if (keywordsFound.length < keywords.length) {
      suggestions.push(`Consider adding some of these keywords if relevant: ${keywords.filter(k => !keywordsFound.includes(k)).join(', ')}`);
    }
    ...
    if (cv.education.length === 0) {
      suggestions.push("Add your educational background for a more complete CV.");
    } else if (cv.education.length < 2) {
      suggestions.push("Consider adding more educational experiences if available.");
    }
    ...
    if (cv.experience.length === 0) {
      ...
      cv.experience.forEach(exp => {
        if (!exp.description || exp.description.split(' ').length < 30) {
          suggestions.push(`Expand the description for your role at ${exp.company}. Aim for at least 30 words.`);
        }
      });
    }
    ...
    if (cv.skills.length < 5) {
      suggestions.push("List more relevant skills to improve your CV's ATS score. Aim for at least 5 skills.");
    }
    ...
    if (!cv.personalInfo.phone) {
      personalInfoScore -= 5;
      suggestions.push("Include your phone number for potential employers to reach you.");
    }
  ...
  return (
    <div className="mt-6 p-6 bg-white rounded-lg shadow-md">
      <h2 className="text-2xl font-bold mb-4">ATS Optimization Analysis</h2>
      <div className="mb-4">
